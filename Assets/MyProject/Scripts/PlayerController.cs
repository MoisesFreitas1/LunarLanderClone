...
public class PlayerController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Vk[1] > 0)
        {
            mainCamera.GetComponent<Camera>().orthographicSize = cameraSize + 0.9f * Vk[1];
            if (mainCamera.GetComponent<Camera>().orthographicSize > 60)
            {
    ...
    public void CollisionDetected()
    {
        if(feetL && feetR)
        {
            if (engineL)
            {
                Instantiate(SmokeL, LThrusters.transform.position + new Vector3(0, -3.4f, 0), Quaternion.Euler(90, LThrusters.transform.rotation.y, LThrusters.transform.rotation.z));
            }
            if (engineR)
            {
                Instantiate(SmokeR, RThrusters.transform.position + new Vector3(0, -3.4f, 0), Quaternion.Euler(90, RThrusters.transform.rotation.y, RThrusters.transform.rotation.z));
            }
            feetL = false;
            feetR = false;
